...
$dir = "e:\软件综合课程设计\IDE\CommunityManagement\uploads\facility"

# Category colors: [start, end]
$colors = @{
    'sport'  = @('#3b82f6', '#1e40af')
    'tool'   = @('#f97316', '#c2410c')
    'ent'    = @('#8b5cf6', '#6d28d9')
...
# Facilities: [filename, label, sublabel, category_key]
$items = @(
...
<text x="200" y="195" text-anchor="middle" font-size="16" fill="rgba(255,255,255,0.7)" font-family="system-ui,sans-serif">$sublabel</text>
</svg>
"@

    $svgPath = Join-Path $dir $fname
    [System.IO.File]::WriteAllText($svgPath, $svg, [System.Text.Encoding]::UTF8)
}

Write-Host "Generated 15 SVG placeholders in $dir"
